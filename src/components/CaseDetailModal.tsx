...
import { Case, EvidenceFile, UserRole, User } from '../types';
import { X, Upload, FileText, Video, Image, Music, Shield, AlertTriangle, Users, Calendar, Clock, Paperclip, Eye, CheckCircle, FileDown, Trash2 } from 'lucide-react';
...
export const CaseDetailModal: React.FC<CaseDetailModalProps> = ({
  ...
  const handleDownloadPdf = () => {
    ...
    const evidenceRows = c.evidence.map((ev, idx) => {
    ...
    const photoPages = c.evidence.filter(ev => ev.url || ev.fileType === 'Image').map((ev, pIdx) => `
      <div style="page-break-before: always; break-before: page; padding-top: 20px;">
        <div style="border-bottom: 2px solid #000; padding-bottom: 8px; margin-bottom: 16px; display: flex; justify-content: space-between; align-items: center;">
          <div>
            ...
            <p style="margin: 4px 0 0 0; font-size: 12px; font-weight: 700; color: #4a5568;">CASE ID: ${c.id} • ${c.title}</p>
          </div>
        ...
        <body>
          <div class="action-bar no-print">
            ...
            <div style="display: flex; gap: 12px;">
              <button class="btn btn-primary" onclick="window.print()">📥 Download PDF / Browse Device Location</button>
            ...
            <div style="margin-bottom: 20px; padding: 16px; border: 1px solid #cbd5e1; border-radius: 8px; background: #f8fafc;">
              <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 12px; font-size: 12px; margin-bottom: 12px;">
                <div>
                  ...
                  <div style="font-size: 14px; font-weight: 900; color: #0f172a;">${c.title}</div>
                </div>
                <div>
                  <div style="font-size: 10px; font-weight: 800; text-transform: uppercase; color: #64748b;">Crime Category & Status</div>
                  <div style="font-size: 13px; font-weight: 800; color: #0f172a;">${c.crimeCategory} (${c.status})</div>
                </div>
                <div>
                ...
                <div>
                  <div style="font-size: 10px; font-weight: 800; text-transform: uppercase; color: #64748b;">Host Administrator</div>
                  <div style="font-weight: 700; color: #0f172a;">${c.assignedHostName}</div>
                </div>
              </div>
            ...
            <div style="margin-bottom: 20px;">
              ...
              <div style="display: grid; grid-template-columns: 1fr 1fr 1fr; gap: 10px; font-size: 12px;">
                <div style="padding: 10px; border: 1px solid #cbd5e1; border-radius: 6px; background: #f8fafc;">
                  ...
                  <span style="font-weight: 800; color: #0f172a;">${c.assignedHostName}</span>
                </div>
                <div style="padding: 10px; border: 1px solid #cbd5e1; border-radius: 6px; background: #f8fafc;">
                  <span style="font-size: 10px; font-weight: 800; text-transform: uppercase; color: #64748b; display: block;">Police Officers (${c.assignedOfficerNames.length})</span>
                  <span style="font-weight: 800; color: #0f172a;">${c.assignedOfficerNames.length > 0 ? c.assignedOfficerNames.join(', ') : 'None assigned'}</span>
                </div>
                <div style="padding: 10px; border: 1px solid #cbd5e1; border-radius: 6px; background: #f8fafc;">
                  <span style="font-size: 10px; font-weight: 800; text-transform: uppercase; color: #64748b; display: block;">Advocates (${c.assignedAdvocateNames.length})</span>
                  <span style="font-weight: 800; color: #0f172a;">${c.assignedAdvocateNames.length > 0 ? c.assignedAdvocateNames.join(', ') : 'None assigned'}</span>
                </div>
              </div>
            </div>
  ...
  return (
    <div className="fixed inset-0 z-[2000] flex items-center justify-center p-3 sm:p-5 pt-16 sm:pt-20 bg-black/80 backdrop-blur-md overflow-y-auto">
      <div
        className={`relative w-full max-w-4xl my-auto rounded-2xl border shadow-2xl overflow-hidden transition-all ${
          themeMode === 'bright'
        ...
        <div className={`p-3.5 sm:p-6 border-b flex flex-col sm:flex-row sm:items-center justify-between gap-3 ${
          ...
          <div>
            <div className="flex flex-wrap items-center gap-1.5 sm:gap-3 mb-1">
              ...
              <span className={`text-[11px] font-bold px-2 py-0.5 rounded ${
                themeMode === 'bright'
                  ? 'bg-black/10 text-slate-950 border border-black/20'
              ...
              <span className={`text-[11px] font-bold px-2.5 py-0.5 rounded-full ${
                c.status === 'Solved'
                  ? 'bg-emerald-500/20 text-emerald-800 sm:text-emerald-400 border border-emerald-500/40 font-black'
                  : c.status === 'Active'
                  ? 'bg-red-500/20 text-red-800 sm:text-red-400 border border-red-500/40 font-black'
                  : 'bg-amber-500/20 text-amber-900 sm:text-amber-400 border border-amber-500/40 font-black'
              }`}>
                ● {c.status}
              </span>
            </div>
          ...
          <button
            ...
            className={`self-end sm:self-center p-1.5 sm:p-2 rounded-full transition-colors ${
              themeMode === 'bright' ? 'bg-black/10 hover:bg-black/20 text-slate-950' : 'hover:bg-slate-700/60 text-slate-400 hover:text-white'
            }`}
          ...
          <div className={`p-4 sm:p-5 border-t flex flex-wrap items-center justify-between gap-3 ${
            ...
            <div className="flex items-center space-x-3">
              ...
              <button
                ...
                className="px-5 py-2.5 bg-gradient-to-r from-amber-500 to-yellow-500 hover:from-amber-400 hover:to-yellow-400 text-slate-950 font-black rounded-xl text-xs flex items-center space-x-2 shadow-lg hover:scale-105 transition-all cursor-pointer"
              >
        ...
        <div className="mb-6 p-4 border border-gray-400 rounded-lg bg-gray-50">
          <div className="grid grid-cols-2 gap-4 text-xs mb-3">
            ...
            <div>
              ...
              <p className="text-sm font-bold text-black">{c.crimeCategory} ({c.status})</p>
            </div>
            <div>
            ...
            <div>
              <p className="text-gray-500 font-bold uppercase text-[10px]">Host Administrator</p>
              <p className="font-semibold text-black">{c.assignedHostName}</p>
            </div>
          </div>
        ...
        <div className="mb-6">
          ...
          <div className="grid grid-cols-3 gap-3 text-xs">
            <div className="p-3 border border-gray-300 rounded bg-gray-50">
              ...
              <span className="font-bold text-black">{c.assignedHostName}</span>
            </div>
            <div className="p-3 border border-gray-300 rounded bg-gray-50">
