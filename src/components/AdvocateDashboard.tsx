...
import { Shield, FolderKanban, Eye, FileText, Upload, Scale } from 'lucide-react';
    ...
    <div className="max-w-7xl mx-auto px-3 sm:px-6 lg:px-8 py-4 sm:py-8 space-y-6 sm:space-y-8">
      ...
      <div className="space-y-4">
        <h2 className={`text-lg sm:text-xl font-bold flex items-center ${themeMode === 'bright' ? 'text-amber-800' : 'text-yellow-400'}`}>
          <FolderKanban className="w-5 h-5 mr-2" /> Cases Assigned to Legal Counsel ({myCases.length})
        </h2>
        ...
        {myCases.length === 0 ? (
          <div className={`p-8 sm:p-12 rounded-3xl border border-dashed text-center space-y-4 max-w-xl mx-auto my-6 ${
            ...
            <p className="text-xs text-slate-400 max-w-md mx-auto">
              You currently have no cases allotted to your Legal Counsel account. Once a Host Inspector or DSP assigns a case to you, legal briefs and evidence files will appear here.
            </p>
          </div>
        ) : (
